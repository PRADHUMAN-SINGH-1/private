...
14. **LLM judge validation.** A 50-example candidate-review sheet and a comparison script are included. The judge now uses the configured OpenAI-compatible base URL, bounded timeouts, retries, and resumable partial outputs. Final validation had no API key in its execution environment, so no external score or agreement statistic is claimed.
