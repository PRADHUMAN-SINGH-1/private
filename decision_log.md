...
3. **Intent taxonomy — 10 issue-level classes plus fallback.** The taxonomy is deliberately small enough to evaluate while covering the dominant AppleSupport themes in the corpus; `other_services` is a runtime fallback rather than an eleventh golden-set class.
...
14. **LLM judge validation.** A 50-example candidate-review sheet and comparison script are included. Final validation completed 50 genuine external scores with recorded model provenance (18 `openrouter/free`, 32 `gemini-3.6-flash`) and a 50-row comparison. Because free-provider limits required a mixed-provider run and kappas were low, it is reported as limited evidence rather than a single-model reliability claim.
